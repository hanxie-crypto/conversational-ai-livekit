...
class SynthesizeStream(tts.SynthesizeStream):
    ...
    def push_text(self, token: str | None) -> None:
        ...
        if not token or len(token) == 0:
            return
        self._text += token
    async def _call_speech_synthesizer(self, tts_str: str,callback: ResultCallback) -> None:
       await SpeechSynthesizer.call(
        ...
        while True:
            try:
                ...
                if(tts_str != STREAM_EOS and tts_str != ""):
                    # await self._call_speech_synthesizer(tts_str,callback)
